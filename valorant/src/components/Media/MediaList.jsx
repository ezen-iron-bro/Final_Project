...
const MediaList = ({ selectedCategory, onPopup }) => {
  const [filteredMedia, setFilteredMedia] = useState([]);

  const onShuffle = (array) => {
    const itemArray = array.slice();
  ...
  useEffect(() => {
    const filteredData = selectedCategory === "All" ? mediaGallery : mediaGallery.filter((item) => item.category === selectedCategory);
    setFilteredMedia(onShuffle(filteredData));
  }, [selectedCategory]);
  //카테고리 변경시 배열 섞기
    ...
    <div className="section-wrap">
      ...
      <div className="container">
        {filteredMedia.map((item) => (
          <MediaItem key={item.id} item={item} onPopup={onPopup} />
        ))}
      </div>
    </div>
  );
