...
import { MediaContainer } from "../styled/valorantStyled";
...
const Media = () => {
  ...
  const [info, setInfo] = useState(mediaGallery);
  ...
  const onPopup = id => {
    setInfo(mediaGallery.find(item => item.id === id));
    setIsPopup(true);
  };
  ...
  return (
    <MediaContainer>
            ...
            <div className="info-text">
              <div>
                <h2 className="media-title">
                  <span className={isActive ? "active" : ""}>미디어</span>
                </h2>
                <div className="media-description">
                  <div className={`des-box ${isActive ? "active" : ""}`}>
                    <p>
                      이 게임은 여러분을 위한 것입니다. 이곳에 올라온 자료는 언론 보도, 콘텐츠 제작 등에 마음껏
                      활용하셔도 좋습니다.
                    </p>
                    <p>
                      이곳에 올라온 파일로 무언가를 만드셨다면 소셜 미디어에서 @PlayVALORANT를 태그해주세요. 여러분이
                      창조해낼 멋진 작품을 기다립니다.
                      <br />
                      <a href="#">법적 고지 사항</a>도 꼭 확인해 주세요.
                    </p>
                  </div>
                </div>
              </div>
      ...
      <section className="media-mid">
        <MediaMenu onCategoryChange={categoryChange} />
      </section>
      <section className="media-bot">
        <MediaList selectedCategory={selectedCategory} onPopup={onPopup} />
      </section>
      <div className="popup">{isPopup && <MediaPopup closePopup={closePopup} info={info} />}</div>
    </MediaContainer>
  );
};
