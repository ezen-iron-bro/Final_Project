...
import { WeaponInfoContainer } from "../../styled/valorantStyled";
import WeaponList from "./WeaponList";
import WeaponSelectBox from "./WeaponSelectBox";

const WeaponInfo = () => {
  const [selectedCategory, setSelectedCategory] = useState("전체 무기");
  ...
  return (
    <WeaponInfoContainer>
      <div className="weapon-detail">
        <span className="weapon-detail-dot1"></span>
        <span className="weapon-detail-dot2"></span>
      </div>
      <div className="weapon-layout">
        <div className="background-logo">
          <div style={{ transform: `translateY(${iconPosition}px)` }}>
      ...
      <div className="weapon-wrapper">
        <div className="wrap-deco-left"></div>
        <div className="wrap-deco-right"></div>
        <div className="title-container">
          <h6 className="vertical">
            <span className="scamble-text">
              <span>한계를</span>
              <span>돌파하라</span>
            </span>
          </h6>
          <h6 className="horizontal">
            <span className="scamble-text">
              <span>발로란트 프로토콜</span>
              <span>MR0C - SWD - BR4</span>
            </span>
          </h6>
          <h2 className="weapon-title">
            <span className={isActive ? "active" : ""}>무기를 선택하세요</span>
          </h2>
          <WeaponSelectBox onCategoryChange={categoryChange} />
        </div>
        <WeaponList selectedCategory={selectedCategory} />
      </div>
    </WeaponInfoContainer>
  );
};
