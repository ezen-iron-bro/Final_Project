...
const MediaItem = ({ item, onPopup }) => {
  const { id, title, category, image } = item;

  const randomNumber = max => Math.floor(Math.random() * max) + 1;
  const columns = randomNumber(2);
  const rows = randomNumber(2);

  const itemStyle = () => {
    const gridColumnValue = rows <= 1 ? 1 : columns;
    return {
      gridRow: `span ${rows}`,
      gridColumn: `span ${gridColumnValue}`,
    };
  };
  //item style 함수 생성
  //grid item의 높이가 1이면 너비도 1이 되도록 지정

  const paths = {
    wallpaper: (
      <path d="M.26 1.48v11.05h13.48V1.48H.26zm12.98.5v5.28l-2.87-3.59L6.9 9.29 4.23 6.61.76 11.48v-9.5h12.48zM.99 12.03 4.3 7.39l2.7 2.7 3.43-5.56 2.81 3.52v3.97H.99z"></path>
    ),
    artwork: (
      <path
        d="M11.36 1.59 9.69.51l-4.11 6.6h-.09c-.03 0-2.59 1.17-2.02 3.63.4 1.74-.78 2.25-.83 2.27l.13.48c.13-.02 3.27-.51 4.62-2.42.52-.74.7-1.6.53-2.55l3.44-6.93zm-1.52-.38.87.56-3.16 6.36-1.5-.83 3.79-6.09zm-2.86 9.57c-.79 1.12-2.33 1.71-3.33 2 .31-.44.54-1.13.31-2.15-.42-1.81 1.17-2.79 1.62-3.02l.44.24 1.42.8c.13.8-.03 1.51-.46 2.13z"
        fill="#c5c8c0"
      ></path>
    ),
    content: (
  ...
  return (
    <figure className={`item item${id}`} style={itemStyle()} onClick={() => onPopup(id)}>
